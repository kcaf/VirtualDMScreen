...
var GridModel = function() {
	var _this = this;
	_this.Selector = "#grid-map";
	_this.ClickY = null;
	_this.ClickX = null;
	_this.Clicked = false;
	_this.Scale = ko.observable(1).extend({ trackChange: true });

	_this.AddToken = function() {
		var div = $("<div class=\"grid-token\">");
		//_this.TokenList.push({Element: div});
...
/*
	_this.MouseMove = function(data, event) {
		_this.Clicked && _this.DragScroll(event);
	};
	...
	_this.MouseUp = function(data, event) {
		_this.Clicked = false;
		$(_this.Selector).css("cursor", "auto");
	};

	_this.DragScroll = function(event) {
		$(_this.Selector).css("cursor", "move");
		var ymod = _this.ClickY - event.pageY > 0 ? 1 : _this.ClickY - event.pageY < 0 ? -1 : 0,
			xmod = _this.ClickX - event.pageX > 0 ? 1 : _this.ClickX - event.pageX < 0 ? -1 : 0,
			scalemod = _this.Scale() * 10;
		$(_this.Selector).parent().scrollTop($(_this.Selector).parent().scrollTop() + (_this.ClickY-event.pageY)/scalemod);
		$(_this.Selector).parent().scrollLeft($(_this.Selector).parent().scrollLeft() + (_this.ClickX-event.pageX)/scalemod);
	};
*/
/*
	_this.Zoom = function(data, event) {
		if(event.deltaY > 0)
			_this.ZoomOut(event);
	...
	_this.ZoomOut = function(event) {
		if(_this.Scale() < 5) {
			...
			_this.Pan(event.offsetX, event.offsetY);
		}
	};
	...
	_this.Pan = function(pointX, pointY) {
		$(_this.Selector).css("transform","translate("+pointX+"px, "+pointY+"px) scale(" + _this.Scale() + ") translate(-"+pointX+"px, -"+pointY+"px)");
		//$("#grid-map").css("transform", "scale(" + _this.Scale() + ")");
		$(_this.Selector).data("scale", _this.Scale());
	};
